...
const Mesh* MeshLibraryBuild(const char* meshName) {
	if (meshName) {
		const Mesh* found = MeshLibraryFind(meshName);
		if (!found) {
			char pathName[FILENAME_MAX] = "";
			sprintf_s(pathName, _countof(pathName), "Data/%s.txt", meshName);
			Stream meshStream = StreamOpen(pathName);
			if (meshStream) {
				Mesh* mesh = MeshCreate();
				MeshRead(mesh, meshStream);
				MeshLibraryAdd(mesh);
				StreamClose(&meshStream);
				return mesh;
			}
		}
		return found;
...
static void MeshLibraryAdd(const Mesh* mesh) {
	if (mesh && meshes.meshCount < meshListSize) {
