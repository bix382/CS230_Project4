...
void TraceInit()
{
	...
	fopen_s(&traceFileHandle, traceFileName, "wt");
	...
	if (!traceFileHandle) {
		printf("Failed to open file. Sorry!");
	}
...
void TraceMessage(const char * formatString, ...)
{
	...
	if (traceFileHandle != NULL)
	{
		...
		vfprintf_s(traceFileHandle, formatString, arg_ptr);
		fprintf_s(traceFileHandle, "\n");
		va_end(arg_ptr);
	}
